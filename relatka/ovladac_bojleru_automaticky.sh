...
# Konstanty - thresholdy pro hysterezi
...
# Spodni urovne dopocitame
LOWER_THRESHOLD_1=$[ $UPPER_THRESHOLD_1 - $HYSTEREZE ]
LOWER_THRESHOLD_2=$[ $UPPER_THRESHOLD_2 - $HYSTEREZE ]
LOWER_THRESHOLD_3=$[ $UPPER_THRESHOLD_3 - $HYSTEREZE ]


# Nejdriv si precteme, jak je na tom aktualne baterie a jak jsou na tom aktualne relatka a na kolik fazi zrovna bezi bojler
BATTERY_CHARGE_STATE=`/home/jimo/rizeni-elektrarny/goodwe/read_goodwe.py | grep ^battery_soc: | cut -d\= -f2 | cut -d\  -f2`
RELAY_STATE=`curl http://$IP_ADDRESS:8080/99 2>/dev/null`
RUNNING_PHASES=`echo $RELAY_STATE | cut -b1-3 | grep -o 1 | wc -l`

# A ted uz samotna logika... jedeme nahoru a dolu s hysterezi, viz thresholdy vyse
...
if [[ $BATTERY_CHARGE_STATE -ge $UPPER_THRESHOLD_3 ]] || ([[ $BATTERY_CHARGE_STATE -ge $LOWER_THRESHOLD_3 ]] && [[ $RUNNING_PHASES -eq 3 ]]); then
	...
	ACTION="Baterie: $BATTERY_CHARGE_STATE %, bojler nyni bezi na 3 faze, predchozi stav $RUNNING_PHASES"
    fi
	...
	[[ `echo $RELAY_STATE | cut -b2` -eq 0 ]] && $SCRIPT_DIR/ovladac_relatek.sh 2 on
	[[ `echo $RELAY_STATE | cut -b1` -eq 0 ]] && $SCRIPT_DIR/ovladac_relatek.sh 3 on
	ACTION="Baterie: $BATTERY_CHARGE_STATE %, bojler nyni bezi na 2 faze, predchozi stav $RUNNING_PHASES"
    fi
	...
	[[ `echo $RELAY_STATE | cut -b1` -eq 0 ]] && $SCRIPT_DIR/ovladac_relatek.sh 3 on
	ACTION="Baterie: $BATTERY_CHARGE_STATE %, bojler nyni bezi na 1 fazi, predchozi stav $RUNNING_PHASES"
    fi
	...
	ACTION="Baterie: $BATTERY_CHARGE_STATE %, bojler je nyni vypnut, predchozi stav $RUNNING_PHASES"
    fi
fi

# Pokud byla zmena, zalogujeme
[[ "$ACTION" != "Default" ]] && echo "`date +%Y-%m-%d_%H:%M` $ACTION" >> $SCRIPT_DIR/../logs/relatka.log
